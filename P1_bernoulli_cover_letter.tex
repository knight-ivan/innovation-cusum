...
Prediction Innovations from Recurrent Networks for Sequential Change-Point
Detection,''} for consideration for publication in \textit{Bernoulli}.
...
sub-Gaussian tail conditions, the Innovation CUSUM enjoys an exponential
in-control average-run-length lower bound and a logarithmic
detection-delay upper bound --- the classical trade-off, model-free with
respect to the data-generating dynamics though not assumption-free.  A
further proposition identifies when learning is statistically useful:
under a nonlinear conditional mean, fixed linear residualization carries
an irreducible innovation defect that a consistent learned predictor
removes.
