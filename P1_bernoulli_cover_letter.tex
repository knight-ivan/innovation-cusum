...
\usepackage[a4paper,top=1.6cm,bottom=1.6cm,left=2.5cm,right=2.5cm]{geometry}
...
logarithmic-delay trade-off, which we show is order-optimal in Lorden's
minimax sense --- and first-order minimax optimal under a Gaussian innovation
shift, becoming exactly optimal once the threshold is calibrated to the
false-alarm constraint; the guarantee is model-free with respect to the
data-generating dynamics though not assumption-free.  Two further propositions identify when learning is
...
neither relies on the other's results.
