...
\title{Martingale Innovations from Contractive Recurrent Networks:
       A Distribution-Free Foundation for Sequential Monitoring}
\runtitle{Martingale Innovations for Sequential Monitoring}
...
any predictor with bounded conditional residual drift and sub-Gaussian
tails, the Innovation CUSUM has a sharp exponential lower bound on the
in-control average run length and a logarithmic upper bound on the detection
delay; together they attain the minimax-optimal detection rate, exactly so
under a Gaussian innovation shift.  The guarantee needs no parametric model
for the data.  We also identify when learning helps: under a nonlinear
...
minimax-optimal detection rate of classical sequential analysis, exactly so
under a Gaussian innovation shift (Theorem~\ref{thm:optimality}). Fourth, we identify
...
approximation part and propagating its decay into the monitoring guarantee.

\begin{proposition}[Rate of the innovation defect and of the guarantee]
\label{prop:whiteness-rate}
  ...
  defect, $\sigma_e^2=\E\|X_{t+1}-m_t\|^2$ for the irreducible one-step Bayes
  risk, and $R(\phi)=\E\|X_{t+1}-g_\phi(h_t)\|^2$ for the population risk (in
  the stationary regime, so constant in $t$).
  \begin{enumerate}[label=(\roman*), itemsep=3pt]
    ...
    \item \textbf{(Rate.)}  Suppose the width-$W$ class $\Phi_W$ approximates
      the Bayes predictor at rate
      $\varepsilon_{\mathrm{app}}(\Phi_W)\le c_{\mathrm A}\,W^{-2\beta}$
      (as available for smooth filters approximated by recurrent/reservoir
      maps, \citep{gonon2020reservoir}) and obeys, for $\beta$-mixing
      in-control data, the excess-risk bound
      $\E\,\varepsilon_{\mathrm{est}}\le c_{\mathrm E}\,W\log T/T$
      \citep{mohri2010stability}.  Balancing at
      $W\asymp(T/\log T)^{1/(2\beta+1)}$ gives
      $\E\,\eta_T = O\!\bigl((T/\log T)^{-\beta/(2\beta+1)}\bigr)
       =\widetilde O\!\bigl(T^{-\beta/(2\beta+1)}\bigr)$.
    \item \textbf{(Effect on the guarantee.)}  Hence the net drift margin
      $\kappa_T=k-\eta_T\uparrow k$, and by Theorem~\ref{thm:arl} the
      guaranteed in-control ARL exponent $2\kappa_T/\sigma^2$ and the
      detection efficiency $2\kappa_T\mu/\sigma^2$ approach their
      Bayes-optimal values at the rate $\eta_T$ of part~(ii); in
      the Gaussian regime of Theorem~\ref{thm:optimality} the efficiency
      approaches the information number $I$ at this rate.
  \end{enumerate}
\end{proposition}
...
\label{thm:arl}
  ...
  sub-Gaussianity parameter.  The exponent $\theta^\ast=2\kappa/\sigma^2$ is
  the Lundberg (adjustment) coefficient of the increment $\hat e_t-k$ and
  \emph{coincides with the classical i.i.d.\ CUSUM rate}; equivalently, an
...
\begin{remark}[Sharpness]
  ...
  $\eta$ increases, becoming vacuous only when $\eta\to k$.  This exponent is
  the best attainable under Assumption~\ref{ass:pathway}: it is the Lundberg
  coefficient and matches the exact exponential rate of the classical
  i.i.d.\ CUSUM \citep{siegmund1985sequential,lorden1971procedures}, so no
  looseness remains in the exponent, and --- in contrast to an elementary
  Chernoff-plus-union-bound estimate --- the pre-exponential constant is
  unity.  The threshold formula $h\geq(\sigma^2/2\kappa)\ln L$ enables
...
\label{cor:twosided}
  ...
  \[
    \E_0[\tau_h^{\pm}]
    \;\geq\; \tfrac{1}{4}\exp\!\left(\frac{2\kappa h}{\sigma^2}\right),
  \]
  with the same sharp exponent $2\kappa/\sigma^2$ as in
  Theorem~\ref{thm:arl}.
...
\label{cor:tradeoff}
  ...
  \[
    ...
    \;\leq\; \frac{1}{\mu}\Bigl(\tfrac{\sigma^2}{2\kappa}\ln L+B\Bigr)
...
\emph{order} of detection delay.  We now record that it is in fact
asymptotically optimal in Lorden's minimax sense, and \emph{exactly} so in
the canonical Gaussian case.  For a stopping time $\tau$ and change-point
...
\label{thm:optimality}
  ...
  \begin{enumerate}[label=(\roman*), itemsep=3pt]
    \item \textbf{(Order optimality.)}  Under Assumption~\ref{ass:pathway}
      and (O1)--(O2),
      \[
        \mathcal J(\tau_{h_\gamma})
        \;\leq\;\frac{\sigma^2}{2\kappa\mu}\,\log\gamma+\frac{B}{\mu}
        \;=\;O(\log\gamma),
      \]
      matching, in order, the universal lower bound
      $\inf_{\tau\in\mathcal C_\gamma}\mathcal J(\tau)\geq(\log\gamma)/I\,(1-o(1))$
      of \citet{lorden1971procedures}, with $I$ the Kullback--Leibler
      information number of the change.  The detector is thus first-order
      rate-optimal, with \emph{detection efficiency} $2\kappa\mu/\sigma^2$.
    \item \textbf{(Exact minimax optimality, Gaussian innovations.)}
      ...
      $2\kappa\mu/\sigma^2=\delta^2/(2\sigma^2)=I$, the Innovation CUSUM
      coincides with the Page likelihood-ratio CUSUM for the change, and it
      is \emph{exactly} optimal for $\inf_{\tau\in\mathcal C_\gamma}\mathcal
      J(\tau)$ \citep{moustakides1986optimal}; in particular
      $\mathcal J(\tau_{h_\gamma})\sim(\log\gamma)/I$.
  \end{enumerate}
\end{theorem}
...
\begin{remark}[Interpretation]
  ...
  the choice maximising $\kappa\mu$ subject to $\kappa+\mu=\delta$.
\end{remark}
...
\begin{figure}[tp]
\centering
\includegraphics[width=0.55\textwidth]{code/figures/S6_pathway_A_delay_profile.png}
...
\includegraphics[width=\linewidth]{code/figures/S8_etf_cusum_partA.png}
\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}\centering
\includegraphics[width=\linewidth]{code/figures/S8_etf_cusum_partB.png}
\end{minipage}
...
minimax optimal in Lorden's sense --- exactly so under a Gaussian innovation
shift.  Propositions~\ref{prop:rnn-resid-bridge} and~\ref{prop:whiteness} link
state stabilization to residual drift and identify when a learned predictor
provably beats a fixed linear filter, and Corollary~\ref{cor:dim} treats the
...
rather than a structural one (Study~CUSUM-EQARL); characterising when
