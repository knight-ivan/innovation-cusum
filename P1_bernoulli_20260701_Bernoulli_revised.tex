...
($\rho:=L_\phi\norm{W_h}*{\mathrm{op}}<1$).
The existence and uniqueness of the stationary causal solution
$(H_t^*)*{t\in\mathbb{Z}}$ satisfying
$H_t^*=\phi(W_hH_{t-1}^*+W_xX_t+b_h)$,
$H_t^*\in\sigma(X_t,X_{t-1},\ldots)$,
and the geometric coupling bound
$\norm{h_t-H_t^*}*{L^2}\leq\rho^t\norm{h_0-H_0^*}*{L^2}$
are proved directly in Proposition~\ref{prop:stationary-causal}
...
$(M_t^*)*{t\in\mathbb{Z}}$ is a strictly stationary, ergodic MDS
\emph{adapted} to $(\F_t)$. Under the one-sided filtration
$\sigma(X_1,\ldots,X*{t-1})$, neither exact stationarity nor adaptedness
would hold because the pre-sample past influences $H_t^*$.
The stationary causal solution is generally a \emph{random} process;
...
\begin{remark}[Connection to Study~CUSUM-DIM]
Corollary~\ref{cor:dim} explains the dimension-scaling pattern in
Study~CUSUM-DIM (Section~\ref{sec:cusum-dim}): at $d=10$, the raw and
whitened CUSUM ARL$*0$ falls to 47 and 42, whereas the Innovation CUSUM
rises to 365.  Equation~\eqref{eq:raw-bias} shows that the normalized
serial-dependence bias $\eta_d^{\mathrm{raw}}$ can remain nonzero as
$d\to\infty$, so a fixed $k$ need not yield a positive drift margin without
recalibration using $\Phi$ and $\Sigma_X$.  This is a calibration issue,
not an impossibility: a raw CUSUM remains valid if $k$ exceeds the bias.
By contrast, the Innovation CUSUM guarantee holds for all sufficiently
large $d$, once
$\eta_d'=4\|\Sigma_0^{-1}\|_{\mathrm{op}}\eta_d^2/d<k$.
\end{remark}
...
across the dimensions of Study~CUSUM-DIM.
...
advantage in Study~CUSUM-DIM is a finite-sample calibration effect rather
than a structural one.  Study~CUSUM-MISSPEC considers a nonlinear,
misspecified process in which learned innovations are genuinely whiter and
detection is faster than with linear whitening, as
Proposition~\ref{prop:whiteness} predicts.
Study~LS-Diag finds that a stationary-trained LSTM forget gate systematically
mis-adapts to smooth non-stationarity, motivating an explicit correction.
Study~J-Surr benchmarks four data-driven surrogates for the predictive-decay
profile $J_t$ (defined in Section~\ref{sec:processes}); only the
...
below one under smooth variation, while all four deteriorate under abrupt
change.
