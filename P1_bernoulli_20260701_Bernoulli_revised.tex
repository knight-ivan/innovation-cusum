...
\label{prop:hp-drift}
  ...
  \begin{enumerate}[label=(H\arabic*), itemsep=2pt]
    \item \textbf{(Bounded inputs.)}  $\norm{X_t}\le B_X$ a.s.; then
      $\norm{h_t}\le C_h$ a.s.\ for an explicit constant $C_h$ (see the proof),
      so $h_t$ and $H_t^*$ lie in the compact set
      $\mathcal H:=\{h:\norm{h}\le C_h\}$.
    ...
    \item \textbf{(Uniform estimation, with high probability.)}  The fitted
      readout $\hat o$ satisfies, with probability at least $1-\delta_T$ over
      the training sample $\mathcal D_T$,
  ...
  $\bar\eta_T:=a_\infty+2\varepsilon_T$; and if $k>\bar\eta_T$ then,
  conditionally on $\mathcal D_T$, Theorem~\ref{thm:arl} gives
  $\E_0[\tau_h\given\mathcal D_T]\ge\exp\{2(k-\bar\eta_T)h/\sigma^2\}$
