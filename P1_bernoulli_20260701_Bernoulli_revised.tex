...
%% Cross-references to the companion document are LIVE: the companion's
%% .aux is read at compile time (only its \newlabel entries are kept), so
%% renumbering can never go stale.  The xr package is NOT used because the
%% imsart class forbids it; this macro reproduces the same behaviour.
%% Cold-start compile order: main -> supplement -> main (then bibtex + two
%% more passes each).  For journal submission (files compiled in
%% isolation), freeze first:  python3 make_frozen_xrefs.py
\makeatletter
\newcommand{\inputcompanionaux}[1]{%
  \begingroup
    ...
    \def\newlabel##1##2{\global\expandafter\def\csname r@##1\endcsname{##2}}%
    \IfFileExists{#1.aux}%
      {\@@input #1.aux\relax}%
      {\typeout{** companion #1.aux not found; run pdflatex on it first}}%
  \endgroup}
