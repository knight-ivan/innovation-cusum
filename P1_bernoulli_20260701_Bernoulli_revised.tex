...
\begin{abstract}
Sequential monitoring is challenging when observations are serially dependent, high-dimensional, and not governed by a trusted parametric model. This paper revisits the innovations principle using a learned predictor: a recurrent network removes the serially predictable component of the data stream, and a CUSUM statistic monitors the resulting prediction innovations. The central goal is to show that the false-alarm and detection-delay guarantees of classical change-point theory extend, essentially intact, to black-box learned predictors.
%
The theoretical development has two parts. First, for contractive recurrent networks, we show that the hidden state admits a Doob decomposition into a predictable component and a martingale-difference innovation. This innovation stabilizes geometrically, and its partial sums satisfy a functional central limit theorem. Second, for any predictor whose conditional residual drift is almost surely bounded and whose residuals have sub-Gaussian tails, we establish an exponential lower bound on the in-control average run length of the Innovation CUSUM, with the optimal exponent uniformly over this class, together with a logarithmic upper bound on detection delay.
%
In the Gaussian mean-shift setting, the proposed statistic reduces to the classical likelihood-ratio CUSUM and is first-order minimax optimal. These guarantees require no parametric model for the observed dynamics, although they are not assumption-free. We therefore provide conditions under which a learning guarantee implies the required drift condition with high probability. We also characterize when learning improves monitoring performance: under a nonlinear conditional mean, linear residualization incurs an irreducible error that a learned predictor can eliminate. Simulations and real-data applications illustrate the theory.
\end{abstract}
...
Data streams requiring monitoring for structural change are now ubiquitous. Intensive-care units track dozens of physiological signals, public-health agencies monitor infectious-disease incidence, industrial systems record hundreds of correlated sensor measurements, and risk desks follow panels of asset returns. In each setting, a detector must determine online whether the data-generating mechanism has changed. The two types of error carry different costs. A \emph{false alarm}---signaling a change while the system remains in control---may trigger an unnecessary clinical intervention, halt a production line, or prompt premature portfolio rebalancing. A missed or delayed detection, by contrast, may leave a deteriorating patient unattended, an equipment fault unresolved, or a financial loss unhedged. The statistical objective is therefore not simply to detect change, but to control false alarms without incurring excessive detection delay.

This trade-off is particularly difficult in the settings considered here. Observations are serially dependent, the dependence structure is generally unknown and seldom specified with sufficient confidence to support a parametric monitoring procedure, and the number of monitored coordinates may be large. This paper studies sequential change-point detection under the combined challenges of unknown serial dependence, limited model specification, and high dimensionality.

Several established approaches address components of this problem. Classical CUSUM procedures are powerful—and, under standard likelihood-ratio formulations, optimal or asymptotically optimal—when observations are independent and the pre- and post-change distributions are correctly specified \citep{page1954continuous,lorden1971procedures}. Offline segmentation methods \citep{killick2012optimal} and Bayesian online change-point procedures \citep{adams2007bayesian} provide flexible alternatives for identifying structural changes. Their standard theoretical guarantees, however, typically depend on assumptions about the data-generating process and do not automatically extend to settings with unknown serial dependence or increasing dimension.
%
Classical time-series models address dependence explicitly. Autoregressive models represent temporal structure through a specified lag order, hidden Markov models encode dependence through a latent finite-state process, and Kalman filters propagate a finite-dimensional Gaussian state estimate \citep{box2015time,hamilton1994time}. These models permit tractable monitoring rules and rigorous guarantees when their structural and distributional assumptions are adequate, but misspecification can distort both false-alarm behavior and detection performance. A less developed objective is therefore to construct a monitoring procedure whose false-alarm guarantees remain valid under weak assumptions on serial dependence while accommodating high- or growing-dimensional observations.


A further strand of the literature monitors \emph{residuals} rather than raw observations. A seminal formulation of real-time monitoring with estimated parameters is given by \citet{chu1996monitoring}, while \citet{bai1994residuals} establishes weak-convergence results for sequential empirical processes of estimated residuals in ARMA models. \citet{lai1995sequential} develops a broader theory of sequential change-point detection for quality-control and dynamical-system settings, and \citet{aue2013structural} survey methods for detecting structural breaks in time series. Much of this literature constructs residuals from a specified parametric model, so its theoretical guarantees depend on the adequacy of that model and its estimation.
%
In a complementary direction, conformal test martingales and e-detectors \citep{vovk2005algorithmic,shin2024edetectors} provide nonparametric, anytime-valid monitoring. Standard conformal constructions rely on exchangeability or related invariance assumptions, whereas e-detectors require valid e-values under the in-control null, often obtained from a known reference distribution or a suitably specified null class. The present paper lies between these two traditions. Like residual-based procedures, it monitors one-step-ahead prediction errors, but the predictor is learned rather than specified parametrically. Like e-detector methods, it does not impose a parametric model on the observed dynamics; instead, its in-control guarantees rely on explicit conditions on the conditional residual drift and tails, and the monitoring statistic is Page’s classical CUSUM.

This paper follows a strategy with a long history in time-series analysis: transform the observations into innovations and then monitor those innovations. The innovations principle states that the one-step-ahead error of the conditional-mean predictor is orthogonal to the observed past—or, probabilistically, forms a martingale-difference sequence—and therefore contains no remaining component that is predictable from that past. This principle underlies Wiener--Kolmogorov prediction theory, the innovations approach to filtering \citep{kailath1968innovations}, residual diagnostics in Box--Jenkins modeling \citep{box2015time}, and residual-based methods for detecting abrupt changes \citep{basseville1993detection}.
%
Under a correctly specified linear-Gaussian state-space model, the principle takes an especially strong form: Kalman-filter innovations are independent Gaussian variables, and a likelihood-based CUSUM constructed from them retains the corresponding classical false-alarm guarantees. Outside this setting, obtaining exact innovations generally requires knowledge of the relevant conditional distribution or predictor—precisely what is unavailable when the dependence structure is unknown.
%
Recurrent neural networks (RNNs) \citep{rumelhart1986learning}, long short-term memory networks (LSTMs) \citep{hochreiter1997long}, gated recurrent units (GRUs) \citep{cho2014learning}, and selective state-space models such as Mamba \citep{gu2023mamba} offer flexible ways to learn predictive state representations from data. Although their architectures still impose a state-update structure, they can learn the effective memory and nonlinear predictive relationships rather than requiring a prespecified time-series model. They therefore provide a natural modern implementation of the innovations principle: use a learned predictor to remove, as far as possible, the serially predictable component of the stream, and monitor the resulting residuals. The question addressed in this paper is which classical CUSUM guarantees remain valid when the predictor is learned rather than specified in advance.

Two questions must be answered. First, in what precise probabilistic sense does a learned recurrent predictor produce innovations, and how rapidly does its state—and hence the distribution of those innovations—stabilize? This determines whether calibration based on a training segment remains valid during subsequent monitoring. Second, what false-alarm and detection-delay guarantees can be established for a CUSUM applied to the learned innovations, and do those guarantees remain valid as the observation dimension grows?


Our results are deliberately two-tier. First, for a basic RNN whose recurrent weight matrix satisfies an operator-norm contraction condition (Assumption~\ref{ass:spectral})—enforceable by spectral normalization \citep{miyato2018spectral}—the hidden state admits a Doob decomposition into a predictable component and a martingale-difference innovation. The innovation converges geometrically to its stationary counterpart, and its partial sums satisfy a functional central limit theorem.

Second, the detection theory is architecture-agnostic. Bayes prediction residuals are martingale differences (Proposition~\ref{prop:predresid}), while fitted residuals are controlled through their conditional mean drift. For contractive RNNs, Proposition~\ref{prop:rnn-resid-bridge} bounds this drift by separating initialization, approximation, and estimation errors. The CUSUM results then require only bounded conditional drift and conditional sub-Gaussian tails, yielding an in-control average-run-length lower bound (Theorem~\ref{thm:arl}), an out-of-control delay upper bound (Theorem~\ref{thm:delay}), and the resulting exponential-false-alarm/logarithmic-delay trade-off (Corollary~\ref{cor:tradeoff}). Thus, the RNN theory helps verify the residual conditions, while the CUSUM guarantees apply to any predictor satisfying them.

The contributions comprise four connected results. First, under operator-norm contractivity, the recurrent hidden state is uniformly ($L^2$)-bounded and admits a Doob decomposition whose martingale-difference innovation converges geometrically to stationarity (Theorem~\ref{thm:mds}); its partial sums satisfy a functional central limit theorem (Corollary~\ref{cor:fclt}). Second, Proposition~\ref{prop:rnn-resid-bridge} transfers this state-level stabilization to a residual approximation bound for recurrent predictors with Lipschitz readouts. Third, under explicit conditional-drift and tail conditions, the Innovation CUSUM satisfies exponential in-control ARL and logarithmic detection-delay bounds (Theorems~\ref{thm:arl} and~\ref{thm:delay}). These attain the classical minimax detection rate; in the Gaussian mean-shift case, the procedure is first-order minimax optimal, and the Page CUSUM calibrated to the false-alarm constraint is exactly Lorden-optimal (Theorem~\ref{thm:optimality}). Fourth, with a nonlinear conditional mean, fixed linear residualization incurs an irreducible innovation defect, whereas a consistent learned predictor eliminates it and thereby improves the guaranteed ARL margin (Proposition~\ref{prop:whiteness} and Remark~\ref{rem:whiteness-arl}). Empirical studies examine these mechanisms under misspecification, short calibration windows, and equal-ARL comparisons with parametric detectors.

The contribution lies in the synthesis. Its ingredients are classical: Doob decomposition; the martingale-difference property of Bayes prediction residuals; geometric forgetting under contractive stochastic recursions \citep{doukhan1994mixing,miller2019stable}; martingale functional central limit theory \citep{hall1980martingale,billingsley1999convergence}; the ($L^2$)-projection characterization of the best linear predictor (Proposition~\ref{prop:whiteness}); and classical CUSUM theory, including exponential-supermartingale false-alarm arguments \citep{page1954continuous,lorden1971procedures}. The novelty is their combination: using contractive recurrent states to construct stabilized innovations, transferring this stability to fitted prediction residuals, and establishing sequential CUSUM guarantees under explicit residual-drift and tail conditions without specifying a parametric model for the observed dynamics.

This perspective differs from work on recurrent architectures that emphasizes stability, expressivity, or efficient sequence modeling \citep{miller2019stable,miyato2018spectral,gonon2020reservoir,gu2021efficiently,gu2023mamba,orvieto2023resurrecting}, as well as neural-tangent-kernel and information-bottleneck analyses of fixed-time representations \citep{jacot2018neural,tishby2015deep,saxe2019information}. Here, the learned state removes predictable serial structure, enabling sequential monitoring under explicit drift, tail, and post-change signal conditions without a parametric model for the dynamics.

Sections~\ref{sec:theory}--\ref{sec:pathway} develop the theory, and Section~\ref{sec:empirical} presents four empirical studies. The Supplementary Material contains eight additional studies, background, and proofs. Table~\ref{tab:assumption-map} summarizes the paper’s two tiers: contractive-RNN innovation theory and architecture-agnostic detection guarantees.
...
$\norm{h_t-H_t^*}_{L^2}\leq\rho^t\norm{h_0-H_0^*}_{L^2}$
are proved directly in Proposition~\ref{prop:stationary-causal}
(Section~\ref{app:stationary-causal} of the Supplementary Material).
Thus, the chain of results in this paper does not depend on any external
or unpublished result.
The companion paper \citet{chang2026backward} studies the complementary
question of hidden-state convergence under the same contraction condition
from a reverse-filtration perspective; none of its results are used here.
We write $\Delta_0:=\norm{h_0-H_0^*}_{L^2}$,
$D_t^*:=\E[H_t^*\given\F_{t-1}]$ (the stationary predictable component),
and $M_t^*:=H_t^*-D_t^*$ (the stationary MDS innovation).
Here, the two-sided filtration used in the setup is essential:
$H_t^*$ is a fixed measurable functional of $(X_s)_{s\leq t}$ and is
therefore $\F_t$-measurable, while $D_t^*$ is given by the same functional
of the shifted path at each $t$ (conditioning additionally on the independent
$\sigma(h_0)$ leaves it unchanged). Consequently,
$(M_t^*)_{t\in\mathbb{Z}}$ is a strictly stationary, ergodic MDS
\emph{adapted} to $(\F_t)$. Under the one-sided filtration
$\sigma(X_1,\ldots,X_{t-1})$, neither exact stationarity nor adaptedness
would hold because the pre-sample past influences $H_t^*$.
The stationary causal solution is generally a \emph{random} process;
it should not be identified with either a deterministic mean-field fixed
point or a tail projection.
}
...
\begin{remark}
Parts~(i)--(iii) extend Proposition~\ref{prop:predresid} to the hidden
state. \textbf{Parts~(iv) and~(v) are new:} Part~(iv) establishes
convergence of the transient innovation at rate $\rho^t$, requiring
burn-in
$\log(\Delta_0/\varepsilon)/(-\log\rho)\approx
\log(\Delta_0/\varepsilon)/(1-\rho)$,
while Part~(v) gives $\Sigma_M^*$ for the functional CLT and
Mahalanobis CUSUM scoring.
\end{remark}


\begin{remark}[Scope of the nondegeneracy condition]
\label{rem:rank}
Condition~\eqref{eq:nondeg} ensures
$\lambda_{\min}(\Sigma_M^*)>0$ by requiring non-zero innovation variance
in every direction.  For linear activations, full row rank
$\mathrm{rank}(W_x)=p$ is sufficient, but it may fail for saturating
nonlinearities on nearly flat regions.  When $d_{\mathrm{in}}<p$, full row
rank is impossible and, under linear or locally linearized activations,
$\mathrm{rank}(\Sigma_M^*)\leq d_{\mathrm{in}}$.  Nonlinear transformations
can exceed this bound, so rank deficiency cannot be inferred from
$\mathrm{rank}(W_x)$ alone.  In practice, positive definiteness should be
checked from pilot data; Corollary~\ref{cor:fclt} assumes it.
\end{remark}
...
The ARL guarantee requires $k>\eta$ and improves with
$\kappa=k-\eta$ (Theorem~\ref{thm:arl}).  Under nonlinearity in mean,
linear residualization leaves an irreducible $L^2$ defect
$\eta_{\mathrm{lin}}>0$, whereas a consistent learned predictor removes it.
Because (B1) uses an essential supremum, this $L^2$ improvement enlarges the
guaranteed margin only with the uniform control of
Proposition~\ref{prop:hp-drift}.  Study~CUSUM-MISSPEC confirms the gap,
with learned innovations markedly whiter than linear residuals.
\end{remark}
...
\begin{remark}%[What (H3) costs]
  (H1)--(H2) are structural, though (H1) restricts attention to bounded inputs,
  a sub-regime of the $L^2$ setting used elsewhere and distinct from the Gaussian
  innovations of Theorem~\ref{thm:optimality}(ii).  (H3) is the substantive
  price, demanding sup-norm rather than $L^2$ estimation error.  On the compact $\mathcal H$ this is the
...
We now establish a rigorous in-control performance guarantee for the Innovation
CUSUM, which applies a Page CUSUM to learned prediction innovations.  This is
the second tier of the theory: exact Bayes residuals form an MDS; fitted
residuals are approximately MDS when their conditional drift is small; and
this residual-level property yields the alarm-rate bound through a
sub-Gaussian exponential-supermartingale argument.  No architecture-specific
contraction assumption enters the CUSUM theorem; the guarantee applies to any
predictor whose residuals satisfy the conditional drift and tail conditions
below.
%
Throughout this section, $(\F_t)$ denotes the observable one-sided filtration
generated by the monitored data and any training information, relative to
which the fitted residuals $\hat e_t$ are adapted.  The two-sided filtration
of Section~\ref{sec:theory} is not used here.
...
The ARL grows exponentially in $h$ with rate $2\kappa/\sigma^2$, where
$\kappa=k-\eta$ is the margin between the reference value and the
in-control conditional drift.  The bound weakens as $\eta$ increases and
becomes vacuous as $\eta\to k$.  This exponent is optimal uniformly over
the class in Assumption~\ref{ass:pathway}: Gaussian increments with mean
$-\kappa$ and variance $\sigma^2$ attain it, matching the exact exponential
rate of the classical i.i.d.\ CUSUM
\citep{siegmund1985sequential,lorden1971procedures}.  Some non-Gaussian laws
may have a larger Lundberg root, so the bound is class-sharp rather than
pointwise sharp.  Its unit prefactor also improves on a
Chernoff-plus-union-bound estimate.  Thus,
$h\geq(\sigma^2/2\kappa)\ln L$ guarantees the target ARL using only
$\eta$ and $\sigma^2$.
\end{remark}
...
The constants in Assumption~\ref{ass:pathway} are calibrated on a held-out
in-control segment, separate from the training and monitored data.  The
theorem requires a bound on the \emph{conditional} drift
$\E[\hat e_t\given\F_{t-1}]$, not merely a small unconditional mean.
Calibration means, autocorrelation tests, residual-on-past regressions, and
the nonlinear predictability checks of Section~\ref{sec:mds} provide
diagnostics for $\eta$, but not a theorem-valid estimate of the essential
supremum in~(B1).  Likewise, the sub-Gaussian proxy $\sigma$ must be bounded
conservatively; when $\hat e_t\in[-B,B]$, one may take $\sigma=B$ as
in~(B2).  Choose $k$ so that $\kappa=k-\eta>0$, and set $h$ from
Theorem~\ref{thm:arl} for the target ARL~$L$.  Upper bounds on $\eta$ and
$\sigma$ are conservative.  Section~\ref{sec:empirical} reports the
sensitivity of ARL$_0$ and delay to $(k,h)$.
\end{remark}
...
and (O1)--(O2),
$\mathcal J(\tau_{h_\gamma})\leq(\sigma^2/2\kappa\mu)\log\gamma+B/\mu
=O(\log\gamma)$.
If, in addition, the conditional log-likelihood ratios satisfy the
conditions of the Lorden lower bound \citep{lorden1971procedures} with information number
$I\in(0,\infty)$, then
$\inf_{\tau\in\mathcal C_\gamma}\mathcal J(\tau)
\geq(\log\gamma)/I\,(1-o(1))$.
Hence $\tau_{h_\gamma}$ is order-optimal.  These bounds imply
first-order optimality when $2\kappa\mu/\sigma^2=I$.
Assumption~\ref{ass:pathway} and (O1)--(O2) alone do not define $I$.


\item \textbf{(Gaussian innovations.)}
  Suppose that the conditional laws of $\hat e_t$ given $\F_{t-1}$ are
  Gaussian with variance $\sigma^2$ and means $\eta$ before the change
  and $\eta+\delta$ after it, where $\delta>0$, and set
  $k=\eta+\delta/2$.  Then $\kappa=\mu=\delta/2$ and
  $2\kappa\mu/\sigma^2=\delta^2/(2\sigma^2)=I$.
  Since the conditional log-likelihood-ratio increment is
  $(\delta/\sigma^2)(\hat e_t-k)$, the Innovation CUSUM is a rescaled
  Page likelihood-ratio CUSUM.  Therefore
  $\mathcal J(\tau_{h_\gamma})\sim(\log\gamma)/I$, so
  $\tau_{h_\gamma}$ is first-order asymptotically minimax optimal.
  Exact Lorden optimality \citep{moustakides1986optimal} holds when the
  Page CUSUM threshold is calibrated so that
  $\E_\infty[\tau]=\gamma$ exactly.  Since $h_\gamma$ guarantees only
  $\E_\infty[\tau_{h_\gamma}]\geq\gamma$, exact optimality is not claimed
  for this analytic threshold.
...
Part~(i) summarizes performance through the quantity
$2\kappa\mu/\sigma^2$, which we call the \emph{detection efficiency}; here
$\kappa=k-\eta$ and $\mu$ are the in-control and post-change margins.  Under a Gaussian innovation mean shift of size
$\delta$, $\kappa+\mu=\delta$, and the midpoint choice
$k=\eta+\delta/2$ maximizes $\kappa\mu$.  It yields
$2\kappa\mu/\sigma^2=\delta^2/(2\sigma^2)=I$, the Gaussian information
number, so the innovation CUSUM attains the optimal first-order rate.
\end{remark}
...
\label{cor:dim}
  Let $\hat e_t\in\R^d$ be the multivariate innovation, $\mu_0=\E[\hat e_t]$,
  $\Sigma_0=\Var(\hat e_t)$, and let
  $\mathcal Z_t:=d^{-1}(\hat e_t-\mu_0)^\top\Sigma_0^{-1}(\hat e_t-\mu_0)-1$
  be the aggregated Mahalanobis score.  Write $u_t:=\hat e_t-\mu_0$ and
  $b_t:=\E[u_t\given\F_{t-1}]$, and assume, uniformly in $d$:
  \begin{enumerate}[label=(D\arabic*), itemsep=2pt]
    \item $\norm{\E[\hat e_t\given\F_{t-1}]}\leq\eta_d=O(1)$ a.s.;
    \item $\E[(u_t-b_t)(u_t-b_t)^\top\given\F_{t-1}]\preceq\Sigma_0$ a.s.;
    \item $\lambda_{\min}(\Sigma_0)\geq c_0>0$, so
      $\norm{\Sigma_0^{-1}}_{\mathrm{op}}=O(1)$;
    \item $\mathcal Z_t$ satisfies Assumption~\ref{ass:pathway}(B2) with
      $\sigma_{Z,d}=O(1)$ (e.g.\ if $\hat e_t$ is uniformly bounded in $d$).
  \end{enumerate}
  Then $\eta_d':=4\norm{\Sigma_0^{-1}}_{\mathrm{op}}\eta_d^2/d=O(d^{-1})\to0$, so
  for a fixed reference value $k$ the net margin $\kappa_d=k-\eta_d'\to k>0$ and,
  for all sufficiently large $d$, the ARL bound~\eqref{eq:arl-lb} holds with
  $\kappa_d\geq k/2$.
\end{corollary}
...
\begin{remark}[Reading (D1)--(D2)]
  (D1) constrains the \emph{whole} innovation vector, not each coordinate
  separately.  (D2) bounds only the conditional \emph{variance} of the centred
  innovation, not the squared drift $b_tb_t^\top$; it is therefore strictly
  weaker than $\E[u_tu_t^\top\given\F_{t-1}]\preceq\Sigma_0$, which would force
  $\E[\mathcal Z_t\given\F_{t-1}]\leq0$ and remove the residual term this result
  is about.
\end{remark}

\begin{corollary}[Raw residualization need not be dimension-stable]
\label{cor:dim-raw}
  Let $X_t=\Phi X_{t-1}+\varepsilon_t$ be a VAR(1) with
  $\varepsilon_t\iid(0,I_d)$, $\norm{\Phi}_{\mathrm{op}}=\rho_{\mathrm{op}}>0$
  and stationary covariance $\Sigma_X$, and form the score from the raw
  observations ($\mu_0=0$, $\Sigma_0=I_d$).  Then
  \begin{equation}
    \E[\mathcal Z_t^{\mathrm{raw}}\given\F_{t-1}]
      =\tfrac1d\norm{\Phi X_{t-1}}^2,
    \label{eq:raw-bias}
  \end{equation}
  the noise term $d^{-1}\mathrm{tr}(I_d)-1$ cancelling exactly.  This bias need
  not vanish as $d\to\infty$: for $\Phi=\rho_{\mathrm{op}}I_d$ the coordinates of
  $X_{t-1}$ are i.i.d.\ stationary AR(1) variables with variance
  $(1-\rho_{\mathrm{op}}^2)^{-1}$, so by the strong law of large numbers
  $d^{-1}\norm{\Phi X_{t-1}}^2\to\rho_{\mathrm{op}}^2/(1-\rho_{\mathrm{op}}^2)>0$
  a.s.  Hence no fixed $k$
  guarantees a positive margin unless it is recalibrated to exceed a bias
  depending on $\Phi$ and the stationary law of $X_{t-1}$---information that raw
  CUSUM calibration does not use.  (For particular $\Phi$ the bias does vanish,
  e.g.\ $\Phi=\mathrm{diag}(\rho_{\mathrm{op}},0,\ldots,0)$; the point is that it
  need not.)
\end{corollary}
...
\label{sec:empirical}
This section presents four studies aligned with the theory.  Section~\ref{sec:theory} provides the contractive-RNN innovation prototype, whereas the LSTM detector relies on the approximate Bayes-residual MDS condition in Assumption~\ref{ass:pathway}, not on LSTM contraction.  The studies first assess this condition through residual-whiteness diagnostics and then evaluate detection.  Study~MDS tests the MDS property (Theorem~\ref{thm:mds}); Study~CUSUM-CP compares LSTM-innovation and classical CUSUM detectors at a fixed threshold, with matched-ARL calibration in Study~CUSUM-EQARL; Study~CUSUM-DIM examines dimension stability (Corollary~\ref{cor:dim}) for $d \in \{1,2,5,10\}$; and Study~ETF applies the full pipeline to US equity sector returns around the 2020 COVID crash.

Eight supporting studies are reported in the Empirical Supplement
...
Study~PELT compares the Innovation CUSUM with the offline PELT detector
across the dimensional settings examined in Study~CUSUM-DIM.
%
Study~CUSUM-EQARL repeats the dimension-scaling comparison under matched
false-alarm levels and estimation budgets, showing that the fixed-threshold
advantage observed in Study~CUSUM-DIM is a finite-sample calibration effect
rather than a structural one. Study~CUSUM-MISSPEC considers a nonlinear,
misspecified process in which the learned innovations are genuinely whiter
than those obtained through linear whitening and lead to faster detection,
as predicted by Proposition~\ref{prop:whiteness}.
%
Study~LS-Diag finds that an LSTM forget gate trained under stationarity
systematically adapts poorly to smooth nonstationarity, motivating an
explicit correction. Study~J-Surr benchmarks four data-driven surrogates
for the predictive-decay profile $J_t$ (defined in
Section~\ref{sec:processes}); only the estimator corrected using the
Kullback--Leibler importance estimation procedure (KLIEP)
\citep{sugiyama2012density} achieves normalized mean-squared error (NMSE)
below one under smooth variation, whereas all four deteriorate under
abrupt change.


All code is available in \texttt{code/} and can be reproduced using
Python~3.9, PyTorch~2.2, and R~4.1 by running
\texttt{python run\_all.py}. Studies~MDS and~CUSUM-CP use
1,000--2,000 independent test sequences per configuration, whereas
Study~CUSUM-DIM uses 1,000 test sequences and 2,000 in-control sequences.
...
($\approx 440$) and both exceed the i.i.d.\ nominal (285), indicating that in-control LSTM 
residuals are slightly under-dispersed relative to the i.i.d.\ calibration assumption, consistent 
with the theoretical ARL lower bound of Theorem~\ref{thm:arl} holding with slack.
...
Taken together, the twelve studies---four in the main text and eight in the Supplementary Material---characterize the finite-sample 
behavior of the hidden-state innovation, the predictive-decay profile $J_t$, and the Innovation CUSUM across synthetic and real settings, 
from univariate autoregressions to non-Gaussian, high-dimensional equity returns. The synthetic studies use many independent replications, 
enabling statistical comparisons. By contrast, Study~ETF and Study~NILE (Section~\ref{app:supp-nile}) 
each analyze a single real change-point event and therefore serve 
as end-to-end demonstrations rather than evidence of a typical real-world advantage.


The dimensional scaling observed at fixed $h$ in Study~CUSUM-DIM is striking: raw and whitened ARL$_0$ collapse as $d$ increases, whereas the Innovation CUSUM's ARL$_0$ rises. This result, however, must be interpreted with care. As Study~CUSUM-EQARL (Section~\ref{app:supp-eqarl} of the Supplementary Material) shows, under matched false-alarm levels and estimation budgets, the three detectors are essentially equivalent on these VAR data. The fixed-$h$ collapse arises because the classical detectors operate at a much less conservative point and estimate high-dimensional nuisance parameters from a short in-control window; it does not indicate a structural deficiency. The serial-dependence bias in Corollary~\ref{cor:dim} is real, but once the reference level is calibrated and the nuisance parameters are well estimated, it does not manifest as a dimensional disadvantage in this regime.
%
The advantage of learning the predictor that survives a fully fair comparison is twofold. First, when the conditional mean is nonlinear, fixed linear residualization retains an irreducible innovation defect, whereas the learned predictor does not; this is predicted by Proposition~\ref{prop:whiteness} and confirmed by the whiteness ordering in Study~CUSUM-MISSPEC. Second, in the practically common short-window regime, the learned predictor avoids the ill-conditioned per-sequence estimation that degrades the classical scores.
%
The in-control guarantee is also sharp. The exponent $2(k-\eta)/\sigma^2$ in Theorem~\ref{thm:arl} is the Lundberg coefficient and matches the exact exponential rate of the classical i.i.d.\ CUSUM \citep[Ch.~3]{siegmund1985sequential}. Here, that rate is obtained through a renewal argument with no looseness in the exponent.


Study~KAL (Section~\ref{app:supp-kal} of the Supplementary Material) shows why high persistence is intrinsically difficult: the analytic decay profile $J_t$ decreases slowly, requiring hidden-state proxies to integrate information over a long history. The LSTM outperforms the EM-Kalman estimator at moderate persistence, but the Kalman estimator regains its advantage when $\phi\approx0.95$. Study~J-Surr (Section~\ref{app:supp-jsurr} of the Supplementary Material) shows that KLIEP-type corrections achieve NMSE below one under smooth nonstationarity but deteriorate under abrupt changes, thereby delineating when density-ratio reweighting can substitute for retraining.

% =============================================================================
\section{Conclusion}
\label{sec:conclusion}
Two practical gaps motivated this work. First, although recurrent networks are widely used for sequential prediction, existing results on their stability and expressivity do not provide the narrower characterization needed here: a martingale-innovation account of the recurrent state that supports calibration of downstream inference under explicit residual conditions, without requiring a complete account of what the hidden state encodes. Second, classical change-point detectors are typically calibrated for independent observations or fully specified parametric models, so their false-alarm guarantees can fail under unknown serial dependence, especially as the observation dimension grows.

To address the first gap, Theorem~\ref{thm:mds} establishes the Doob decomposition
$h_t=D_t+M_t$, together with an explicit geometric $L^2$ innovation bound and
stationary covariance. Corollary~\ref{cor:fclt} establishes a functional central
limit theorem for the accumulated innovations.
%
To address the second gap, Theorems~\ref{thm:arl} and~\ref{thm:delay} provide a
sharp in-control ARL lower bound at the Lundberg rate and a matching
detection-delay upper bound. Together, they yield the
exponential-false-alarm/logarithmic-delay trade-off in
Corollary~\ref{cor:tradeoff} for a detector that imposes no parametric model on
the observation dynamics. Theorem~\ref{thm:optimality} shows that this trade-off
is order-optimal under Lorden's minimax criterion and first-order minimax
optimal under a Gaussian innovation shift; it is exact once the threshold is
calibrated to the false-alarm constraint.
%
Propositions~\ref{prop:rnn-resid-bridge} and~\ref{prop:whiteness} link state
stabilization to residual drift and characterize when a learned predictor
achieves a larger guaranteed drift margin---and hence a larger guaranteed ARL
exponent---than a fixed linear filter. Corollary~\ref{cor:dim} extends the
analysis to the multivariate score. Empirical studies of synthetic
autoregressive and vector-autoregressive processes, a nonlinear misspecified
process, the Nile discharge series, and US equity-sector returns around the
2020 COVID crash illustrate these mechanisms.


What appears to be new is the \emph{combination} of these ingredients: to our knowledge, no prior work combines recurrent-state innovation theory with sequential CUSUM calibration to provide model-free monitoring guarantees for a detector built on a learned predictor.

Several questions remain open. The apparent dimension-scaling advantage at a fixed threshold is, under a fully matched comparison, a finite-sample calibration effect rather than a structural one (Study~CUSUM-EQARL). Characterizing when short-window nuisance estimation favors the learned score therefore remains open, as does developing a fully nonstationary theory beyond the locally stationary approximation of Corollary~\ref{cor:ls}. The in-control exponent is already sharp (Theorem~\ref{thm:arl}); the remaining slack lies in the constant of the delay bound, which could potentially be sharpened using the overshoot distribution of the reflected random walk. Another natural extension is to relax the sub-Gaussianity and conditional-homoskedasticity assumptions of Corollary~\ref{cor:dim} to accommodate heavy-tailed or heteroskedastic innovations, with direct relevance to finance.

More broadly, the question of what a learned sequential state encodes probabilistically extends beyond RNNs to transformer attention states, diffusion time-series latents, and language-model context representations. Extending the martingale characterization to such representations could place inference and monitoring for these models on the same rigorous footing.
