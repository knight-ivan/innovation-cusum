...
\label{thm:optimality}
  ...
  \begin{enumerate}[label=(\roman*), itemsep=3pt]
    \item \textbf{(Order optimality.)}  Under Assumption~\ref{ass:pathway}
      ...
      information number of the change.  The detector is thus
      order-optimal (rate-optimal in $\log\gamma$), with \emph{detection
      efficiency} $2\kappa\mu/\sigma^2$; first-order (constant-matching)
      optimality holds only when this efficiency equals $I$, i.e.\ in the
      Gaussian case of part~(ii) below.
...
order-optimal in Lorden's minimax framework, and exactly minimax optimal under
a Gaussian innovation shift.  Propositions~\ref{prop:rnn-resid-bridge} and~\ref{prop:whiteness} link
