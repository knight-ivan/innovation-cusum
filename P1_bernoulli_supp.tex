...
\begin{proof}[Proof of Corollary~\ref{cor:ls} in the main paper]
  ...
  The conditions $b_T\to 0$, $m_T\to\infty$, $m_T/T\to 0$ are both necessary
  and sufficient for the approximation bound to be $o(1)$.
\end{proof}
...
\begin{proof}[Proof of Corollary~\ref{cor:dim} in the main paper]
  ...
  \]
  (The factor of~4 is absorbed into the $O(\cdot)$ notation in the
  corollary statement, where $\eta_d'=O(\|\Sigma_0^{-1}\|_{\mathrm{op}}\eta_d^2/d)$.)
...
\begin{proof}
  ...
  \leq\rho^t(\norm{H_0^{(1)}}_{L^2}+\norm{H_0^{(2)}}_{L^2})\to 0$
  by stationarity, so they are $L^2$-equal.
