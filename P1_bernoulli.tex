...
component that is $L^2$-orthogonal to the entire past of the process.
...
By Proposition~\ref{prop:predresid}, exact Bayes residuals form an MDS, and
by Corollary~\ref{cor:fclt} the hidden-state innovations $M_t$ satisfy a
functional CLT; fitted prediction residuals inherit the approximate MDS
property (Proposition~\ref{prop:predresid}).  The key consequence for the
...
progression; six are reported here and two supporting studies (LS-Diag and J-Surr) are deferred to the Supplementary Material.  Study~MDS examines the MDS property (Theorem~\ref{thm:mds})
...
($\approx 440$) and both exceed the i.i.d.\ nominal (285), indicating
conservative false-alarm control consistent with the fact that LSTM residuals behave near-independently but not exactly independently in the in-control regime ---
...
The negative delay should nonetheless be interpreted cautiously: the S\&P 500 sell-off began in early February, two weeks before the officially designated February 19 peak, so the true onset of the distributional shift is itself uncertain and the pre-$\tau^*$ alarm may reflect genuine early-warning signals in the LSTM innovation process.  M1 and M2 alarm at $t=36$
...
To address the second gap, Theorem~\ref{thm:arl} gives a rigorous
in-control average run length (ARL) lower bound for the Innovation CUSUM,
and Corollary~\ref{cor:dim} formalizes why this guarantee is dimension-stable
...
can be shown to fail in the VAR setting of Corollary~\ref{cor:dim}.
