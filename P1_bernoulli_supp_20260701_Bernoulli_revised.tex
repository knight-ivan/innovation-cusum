...
Prop~\ref{prop:whiteness-rate} & Fitted-predictor defect $\eta_T$
  & \emph{assumed} approximation rate $W^{-2s}$; $\beta$-mixing excess-risk rate
  & $L^2$ defect rate $\eta_T=\widetilde O(T^{-s/(2s+1)})$ only; does \emph{not}
    imply (B1) \\
Prop~\ref{prop:hp-drift} & Fitted residual drift
  & bounded inputs/initial state; uniform approximation; high-probability
    sup-norm estimation; conditional (B2)
  & verifies (B1) with $\bar\eta_T$ w.p.\ $\ge1-\delta_T$; conditional ARL
    bound after burn-in \\
\midrule
...
Thm~\ref{thm:optimality} & Innovation CUSUM score
  & Assumptions above $+$ KL information number; Gaussian shift for part~(ii)
  & Lorden order-optimality; first-order optimality at $h_\gamma$ (Gaussian),
    exact only after exact ARL calibration \\
Cor~\ref{cor:dim} & Mahalanobis innovation score
  & uniform-in-$d$ (D1)--(D4): bounded $\eta_d$, $\lambda_{\min}(\Sigma_0)\ge c_0$,
    conditional homoskedasticity, $O(1)$ score sub-Gaussianity
  & dimension-stable ARL \\
Cor~\ref{cor:dim-raw} & Raw (unlearned) Mahalanobis score
  & VAR(1) observations, identity standardization
  & conditional bias $d^{-1}\norm{\Phi X_{t-1}}^2$ need not vanish as
    $d\to\infty$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{proof}[Proof of Lemma~\ref{lem:equiv}]
  \textbf{Part (a).}
...
\begin{proof}[Proof of Theorem~\ref{thm:mds} in the main paper]
  ...
  term $\norm{\phi(0)}/(1-\rho)$ is added to $C_0$---undamped by $L_\phi$,
  since $\norm{\phi(u)}\leq\norm{\phi(0)}+L_\phi\norm{u}$---consistent with the
...
\begin{proof}[Proof of Proposition~\ref{prop:whiteness-rate} in the main paper]
  ...
  (ii) is the stated balance of the two assumed rates.  (iii) is the
  observation that $\eta_T$ is an $L^2$ norm while
  Assumption~\ref{ass:pathway}(B1) constrains
  $\operatorname*{ess\,sup}_t\E[\hat e_t\mid\F_{t-1}]$; since
  $\|\cdot\|_{L^2}\leq\operatorname*{ess\,sup}|\cdot|$ but not conversely, no
  bound on $\eta_T$ implies (B1), and $\eta_T$ cannot replace $\eta$ in
  Theorem~\ref{thm:arl}.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:hp-drift} of the main paper (High-Probability Verification of the Drift Condition)}
\label{app:proof-hp-drift}

\begin{proof}[Proof of Proposition~\ref{prop:hp-drift} in the main paper]
  \emph{Step 1 (compact reachable set).}  By (H1) and Lipschitzness of $\phi$,
  $\norm{h_t}\leq\norm{\phi(0)}+L_\phi\bigl(\norm{W_h}_{\mathrm{op}}\norm{h_{t-1}}
  +\norm{W_x}_{\mathrm{op}}B_X+\norm{b_h}\bigr)
  =\norm{\phi(0)}+\rho\norm{h_{t-1}}+L_\phi(\norm{W_x}_{\mathrm{op}}B_X+\norm{b_h})$
  a.s.  Iterating this affine recursion and using $\rho<1$ gives
  $\norm{h_t}\leq C_h$ a.s.\ for every $t$, with
  \[
    C_h:=\max\Bigl\{B_0,\;
      \frac{\norm{\phi(0)}+L_\phi\bigl(\norm{W_x}_{\mathrm{op}}B_X+\norm{b_h}\bigr)}{1-\rho}\Bigr\},
  \]
  the same
  bound holds for the stationary causal solution $H_t^*$, which is driven by
  the same bounded inputs.  Hence $h_t,H_t^*\in\mathcal H$ a.s.

  \emph{Step 2 (pathwise coupling).}  Both $h_t$ and $H_t^*$ satisfy the same
  recursion driven by the same input path, so under
  Assumption~\ref{ass:spectral} the map is a $\rho$-contraction in the state
  argument and
  $\norm{h_t-H_t^*}\leq\rho\norm{h_{t-1}-H_{t-1}^*}\leq\cdots\leq
  \rho^{t}\norm{h_0-H_0^*}\leq\rho^{t}\Delta_0^\infty$ a.s.  (This is the
  pathwise form of the $L^2$ coupling used elsewhere; no expectation is taken.)

  \emph{Step 3 (decomposition).}  By Proposition~\ref{prop:whiteness}(i),
  $\E[\hat e_{t+1}\mid\F_t]=m_t-\hat m_t$ with $\hat m_t=\hat o(h_t)$.  Insert
  $o_\star(H_t^*)$ and $o_\star(h_t)$:
  \[
    |m_t-\hat o(h_t)|
    \;\leq\;\underbrace{|m_t-o_\star(H_t^*)|}_{\leq\,a_\infty\ \text{by (H2)}}
    +\underbrace{|o_\star(H_t^*)-o_\star(h_t)|}_{\leq\,L_o\norm{H_t^*-h_t}}
    +\underbrace{|o_\star(h_t)-\hat o(h_t)|}_{\leq\,\varepsilon_T\ \text{on the event, by (H3)}} .
  \]
  The middle term is at most $L_o\rho^t\Delta_0^\infty$ by Step~2, and the last
  term is bounded because $h_t\in\mathcal H$ a.s.\ by Step~1, so the sup-norm
  bound of (H3) applies pointwise.  All three bounds hold almost surely on the
  training event, giving the displayed inequality.

  \emph{Step 4 (verification of (B1) and the conditional ARL).}  For $t\geq t_0$
  the transient term is at most $\varepsilon_T$, so
  $\E[\hat e_{t+1}\mid\F_t]\leq a_\infty+2\varepsilon_T=\bar\eta_T$ a.s.  This is
  Assumption~\ref{ass:pathway}(B1) with $\eta=\bar\eta_T$, but \emph{only from
  $t_0$ onward}: residuals before the burn-in need not satisfy it, which is why
  the monitor is restarted at $t_0$ with $S_{t_0}=0$ and alarm time
  $\tau_h^{(t_0)}$.  By (H3) the training sample is independent of the monitored
  stream (equivalently $\mathcal D_T\subseteq\F_0$), so conditionally on
  $\mathcal D_T$ the fitted readout is a fixed function; (B2) holds conditionally
  by (H4).  Hence the hypotheses of Theorem~\ref{thm:arl} are satisfied
  conditionally on $\mathcal D_T$ for the process $(\hat e_t)_{t>t_0}$, and
  applying that theorem from time $t_0$ with $\kappa=k-\bar\eta_T>0$ gives
  $\E_0[\tau_h^{(t_0)}-t_0\mid\mathcal D_T]\geq\exp\{2(k-\bar\eta_T)h/\sigma^2\}$
  on the training event, whose probability is at least $1-\delta_T$. $\square$
\end{proof}
...
\begin{proof}[Proof of Corollary~\ref{cor:twosided} in the main paper]
  ...
  \[
    \Prob_0(\tau_h^+\leq n)\;\leq\;\Prob_0(N^+\leq n)
      \;\leq\;1-(1-e^{-\theta^\ast h})^{\,n}\;\leq\;n\,e^{-\theta^\ast h},
  \]
...
\begin{proof}[Proof of Theorem~\ref{thm:optimality} in the main paper]
  ...
  transfers.  This exact-optimality conclusion concerns the likelihood-ratio
  CUSUM threshold calibrated so that $\E_\infty[\tau]=\gamma$ exactly; for the
  analytic threshold $h_\gamma$, which only guarantees
  $\E_\infty[\tau_{h_\gamma}]\geq\gamma$, solely the first-order conclusion is
  asserted.  The identities $\kappa=\mu=\delta/2$ and
  $I=\delta^2/(2\sigma^2)$ are direct computations.
\end{proof}

\subsection{Proofs of Corollaries~\ref{cor:dim} and~\ref{cor:dim-raw} of the main paper (Dimension Stability)}
\label{app:proof-cordim}

\begin{proof}[Proofs of Corollaries~\ref{cor:dim} and~\ref{cor:dim-raw} in the main paper]
  \textbf{Corollary~\ref{cor:dim} (innovation CUSUM aggregated score).}
  ...
  (Assumption~(B1) in the vector form (D1) of
  ...
  \textbf{Corollary~\ref{cor:dim-raw} (raw CUSUM collapse).}
  ...
  The convergence asserted in Corollary~\ref{cor:dim-raw} is in fact pathwise,
  not merely in mean.  Take the innovation coordinates independent (e.g.\
  $\varepsilon_t\sim\mathcal N(0,I_d)$).  With $\Phi=\rho_{\mathrm{op}}I_d$ the
  recursion decouples into $d$ independent scalar AR(1) processes
  $X_{t,i}=\rho_{\mathrm{op}}X_{t-1,i}+\varepsilon_{t,i}$, so the coordinates
  $X_{t-1,1},\ldots,X_{t-1,d}$ are i.i.d.\ with stationary variance
  $(1-\rho_{\mathrm{op}}^2)^{-1}$.  Hence, by the strong law of large numbers
  applied across coordinates,
  \[
    \frac{1}{d}\norm{\Phi X_{t-1}}^2
      = \rho_{\mathrm{op}}^2\cdot\frac{1}{d}\sum_{i=1}^{d}X_{t-1,i}^2
      \xrightarrow[\;d\to\infty\;]{\text{a.s.}}
      \rho_{\mathrm{op}}^2\,\E[X_{t-1,1}^2]
      = \frac{\rho_{\mathrm{op}}^2}{1-\rho_{\mathrm{op}}^2}>0,
  \]
  which is the almost-sure limit stated in the corollary.
...
Corollary~\ref{cor:dim-raw}: the serial-dependence bias it exhibits is real, but
