...
\begin{proof}[Proof of Proposition~\ref{prop:hp-drift} in the main paper]
  ...
  exactly Assumption~\ref{ass:pathway}(B1) with $\eta=\bar\eta_T$.  Conditionally
  on $\mathcal D_T$ the fitted readout is fixed, so (B2) and the hypotheses of
  Theorem~\ref{thm:arl} hold conditionally; applying that theorem with
  $\kappa=k-\bar\eta_T>0$ gives the stated conditional ARL bound, valid on the
  training event, whose probability is at least $1-\delta_T$ by (H3). $\square$
\end{proof}
