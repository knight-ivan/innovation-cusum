...
% Cross-references to main paper (auto-generated from main .aux)
\makeatletter
\expandafter\def\csname r@cor:dim\endcsname{{3.9}{12}{Dimension-stable ARL}{theorem.3.9}{}}
\expandafter\def\csname r@cor:fclt\endcsname{{2.8}{6}{Functional CLT for MDS innovations}{theorem.2.8}{}}
\expandafter\def\csname r@cor:ls\endcsname{{2.14}{9}{Approximate local stationary decomposition}{theorem.2.14}{}}
\expandafter\def\csname r@cor:tradeoff\endcsname{{3.7}{12}{Exponential-ARL / logarithmic-delay trade-off}{theorem.3.7}{}}
\expandafter\def\csname r@cor:twosided\endcsname{{3.5}{11}{Two-sided Innovation CUSUM}{theorem.3.5}{}}
\expandafter\def\csname r@eq:Mbound\endcsname{{1}{5}{MDS Decomposition with Stabilization Rate}{equation.2.1}{}}
\expandafter\def\csname r@eq:local-approx-bound\endcsname{{9}{9}{Approximate local stationary decomposition}{equation.2.9}{}}
\expandafter\def\csname r@eq:nondeg\endcsname{{5}{6}{MDS Decomposition with Stabilization Rate}{equation.2.5}{}}
\expandafter\def\csname r@lem:equiv\endcsname{{2.1}{4}{Equivalence of forward and tail limits}{theorem.2.1}{}}
\expandafter\def\csname r@prop:predresid\endcsname{{2.9}{7}{Bayes prediction residuals are martingale differences}{theorem.2.9}{}}
\expandafter\def\csname r@prop:rnn-resid-bridge\endcsname{{2.10}{7}{Bridge from recurrent states to prediction innovations}{theorem.2.10}{}}
\expandafter\def\csname r@prop:whiteness\endcsname{{2.11}{8}{Innovation defect and the misspecification gap}{theorem.2.11}{}}
\expandafter\def\csname r@sec:empirical\endcsname{{4}{13}{Empirical Studies and Discussion}{section.4}{}}
\expandafter\def\csname r@sec:nonstationary\endcsname{{2.3}{8}{Extension to Non-Stationary Processes}{subsection.2.3}{}}
\expandafter\def\csname r@sec:pathway\endcsname{{3}{9}{Innovation CUSUM: In-Control False-Alarm Theory}{section.3}{}}
\expandafter\def\csname r@sec:theory\endcsname{{2}{3}{Methodology}{section.2}{}}
\expandafter\def\csname r@thm:arl\endcsname{{3.2}{10}{In-control ARL lower bound for the Innovation CUSUM}{theorem.3.2}{}}
\expandafter\def\csname r@thm:delay\endcsname{{3.6}{11}{Out-of-control detection-delay upper bound}{theorem.3.6}{}}
\expandafter\def\csname r@thm:mds\endcsname{{2.5}{5}{MDS Decomposition with Stabilization Rate}{theorem.2.5}{}}
\makeatother
...
\begin{proof}[Proof of Theorem~\ref{thm:arl} in the main paper]
  We work throughout under Assumptions~(B1)--(B3) of the main paper.
  ...
  Write $g(\lambda) := \lambda\eta + \tfrac{1}{2}\lambda^2\sigma^2 - \lambda k
  = -\lambda\kappa + \tfrac{1}{2}\lambda^2\sigma^2$ for the drift term in
  $\Lambda_t^{(s)}(\lambda)$.  Since
  $\exp\{\lambda\sum_{j=s+1}^t(\hat e_j - k)\}
   = \Lambda_t^{(s)}(\lambda)\,e^{(t-s)g(\lambda)}$,
  any $\lambda$ with $g(\lambda)\leq 0$ yields a bound that is uniform in
  the segment length $t-s$; the condition $g(\lambda)\leq 0$ holds exactly
  for $0<\lambda\leq 2\kappa/\sigma^2$, and the largest admissible tilt
  $\lambda^* = 2\kappa/\sigma^2$ (the positive zero of $g$, rather than its
  minimizer $\kappa/\sigma^2$) gives the best exponent.
  With $g(\lambda^*)=0$,
  $\exp\{\lambda^*\sum_{j=s+1}^t(\hat e_j - k)\} = \Lambda_t^{(s)}(\lambda^*)$,
  so by Markov's inequality and the supermartingale bound
  $\E_0[\Lambda_t^{(s)}(\lambda^*)] \leq 1$:
  \[
    \Prob_0\!\left(\sum_{j=s+1}^t(\hat{e}_j - k) \geq h\right)
    \leq e^{-\lambda^*h} = e^{-2\kappa h/\sigma^2}.
  \]

  \textbf{Step 3: CUSUM representation and union bound.}
  By the standard recursive argument \citep[p.~20]{siegmund1985sequential}, the one-sided CUSUM satisfies
  ...
  \begin{equation}
    \Prob_0(\tau_h \leq n)
    \leq n^2\,e^{-2\kappa h/\sigma^2}
    \qquad \text{for all }n \geq 1.
    \label{eq:supp-pbound}
  \end{equation}

  \textbf{Step 4: ARL lower bound.}
  Set $c := e^{-2\kappa h/\sigma^2}$ and $N := \lfloor (2c)^{-1/2}\rfloor$.
  For $0 \leq n \leq N$, $n^2 c \leq 1/2$, so $\Prob_0(\tau_h > n) \geq 1/2$.
  Since $N+1 \geq \tfrac{1}{\sqrt{2}}e^{\kappa h/\sigma^2}$,
  \[
    \E_0[\tau_h]
    = \sum_{n=0}^{\infty} \Prob_0(\tau_h > n)
    \geq \frac{N+1}{2}
    \geq \frac{1}{2\sqrt{2}}\,e^{\kappa h/\sigma^2}.
  \]
\end{proof}
...
\begin{proof}[Proof of Corollary~\ref{cor:twosided} in the main paper]
  ...
  segment-sum bound of Step~2 of the proof of Theorem~\ref{thm:arl}
  applies verbatim to $-\hat e_t$.
  A bound of the form $\E_0[\tau_h^{\pm}]\geq\E_0[\tau_h]/2$ does \emph{not}
  follow from a union bound on $\tau_h^+$ and $\tau_h^-$ individually, since
  the expectation of a minimum of two stopping times is not controlled by
  the average of their individual expectations.  Instead, the union-bound
  argument of Step~3 is re-run on the combined event.
  Writing $c:=e^{-2\kappa h/\sigma^2}$, the segment-sum bound gives
  $\Prob_0(\tau_h^+\leq n)\leq n^2c$ and, by the mirrored argument,
  $\Prob_0(\tau_h^-\leq n)\leq n^2c$; a union bound over the two events
  then gives
  \[
  ...
  \]
  Setting $N':=\lfloor(4c)^{-1/2}\rfloor$, for $0\leq n\leq N'$ we have
  $2n^2c\leq1/2$, so $\Prob_0(\tau_h^{\pm}>n)\geq1/2$; since
  $N'+1\geq\tfrac{1}{2}e^{\kappa h/\sigma^2}$,
  \[
    \E_0[\tau_h^{\pm}]
    = \sum_{n=0}^{\infty}\Prob_0(\tau_h^{\pm}>n)
    \geq \frac{N'+1}{2}
    \geq \frac{1}{4}\exp\!\bigl(\kappa h/\sigma^2\bigr).
  \]
  The exponent $\kappa/\sigma^2$ matches the one-sided bound; the
  pre-exponential constant $1/4$ is obtained directly from the union bound
  rather than from a halving of $\E_0[\tau_h]$.
\end{proof}
...
\begin{proof}[Proof of Corollary~\ref{cor:dim} in the main paper]
  ...
  Write $\tilde{u}_t := \hat{e}_t - \mu_0 - b_t$ (the zero-mean part;
  $\E[\tilde{u}_t\given\F_{t-1}] = 0$ by construction; note $\tilde{u}_t \neq u_t$
  in the corollary, where $u_t = \hat{e}_t - \mu_0$).
  Since $b_t b_t^\top \succeq 0$,
  \[
    \E[\tilde{u}_t\tilde{u}_t^\top\given\F_{t-1}]
    = \E[(\hat{e}_t-\mu_0)(\hat{e}_t-\mu_0)^\top\given\F_{t-1}] - b_t b_t^\top
    \preceq \E[(\hat{e}_t-\mu_0)(\hat{e}_t-\mu_0)^\top\given\F_{t-1}]
    \preceq \Sigma_0,
  \]
  the last step by the conditional homoskedasticity assumption on $u_t = \hat{e}_t-\mu_0$
  in the corollary.
...
This section collects the seven supporting empirical studies referenced in
...
is confounded in two ways that both happen to favour the innovation
...
Produced by \texttt{code/study\_equal\_arl\_tier2.py} (per-sequence regime)
and \texttt{code/study\_equal\_arl\_estimation.py} (per-sequence vs.\ global),
...
\subsubsection*{Reproducibility}
Produced by \texttt{code/study\_misspecified.py} (\texttt{nlrot} mode),
writing \texttt{code/results/misspecified\_nlrot.csv}.
